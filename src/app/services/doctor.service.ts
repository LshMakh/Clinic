...
export class DoctorService {
  ...
  getCategoryCount(categoryName: string): Observable<number> {
    return this.http.get<any>(
      `https://localhost:7226/api/Doctor/GetSpecialtyCount/specialty-count/${categoryName}`
    );
  }
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          ...
          const errorMessage =
            error.error?.message || 'Failed to update doctor information';
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          return throwError(
            () =>
              new Error('Failed to extract CV text. Please try again later.')
          );
        })
      );
  }
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          ...
          return throwError(
            () =>
              new Error('Failed to load doctor cards. Please try again later.')
          );
        })
      );
  }
  ...
  private handleDoctorDeletion(id: number): void {
    ...
    if (currentUserId) {
      ...
      if (userPinnedDoctors[currentUserId]) {
        userPinnedDoctors[currentUserId] = userPinnedDoctors[
          currentUserId
        ].filter((pinnedId) => pinnedId !== id);
        localStorage.setItem(
          this.PINNED_DOCTORS_KEY,
          JSON.stringify(userPinnedDoctors)
        );
      }
    }
  ...
  togglePin(doctorId: number): void {
    ...
    if (userPins.includes(doctorId)) {
      userPinnedDoctors[currentUserId] = userPins.filter(
        (id) => id !== doctorId
      );
    } else {
